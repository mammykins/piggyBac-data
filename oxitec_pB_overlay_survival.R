...
yvar <- master[,"g0.lambda"] #yvar is survival
xvar <- master[,"surv"] #xvar is transgenesis efficiency
...
b3 <- ggplot(xy, aes(zvar, xvar)) + geom_boxplot(width=0.8,fill=order, outlier.shape=8) + geom_jitter(alpha=I(0.5), size = 2.2, position=position_jitter(width=0.2, height=0))+
xlab("Species") + ylab("Survival") +
  ylim(0, 0.65) +
  theme_grey(base_size = 18) + theme(legend.position = "none")+
  scale_x_discrete(limits=c("aa", "ae","mf","of","ds","dbm","pbw","tlm","tc"),
  ...
  theme(axis.title.x = element_text(face="bold", colour="grey", size=20),
        ...
        axis.text.x = element_text(angle=90, face="italic", colour="black",size=12,vjust=0.5)) 

b3 #+ theme(panel.background = element_blank()) #this removes background

###output ready for Office
ggsave('surv_overlay.png', dpi = 300) #weird but they seem to be the wrong way round
print(b3)
dev.off()